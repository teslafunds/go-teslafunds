...
// Service implements a Dubaicoin netstats reporting daemon that pushes local
...
type Service struct {
	...
	les    *les.LightDubaicoin // Light Dubaicoin service if monitoring a light node
...
func New(url string, ethServ *eth.Ethereum, lesServ *les.LightDubaicoin) (*Service, error) {
...
func (s *Service) login(in *json.Decoder, out *json.Encoder) error {
	...
	if info := infos.Protocols["dbix"]; info != nil {
		network = strconv.Itoa(info.(*eth.EthNodeInfo).Network)
		protocol = fmt.Sprintf("dbix/%d", eth.ProtocolVersions[0])
	} else {
