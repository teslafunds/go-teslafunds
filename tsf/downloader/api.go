...
<<<<<<< HEAD:tsf/downloader/api.go
// Progress gives progress indications when the node is synchronising with the Teslafunds network.
...
=======
// Syncing provides information when this nodes starts synchronising with the Dubaicoin network and when it's finished.
...
<<<<<<< HEAD:tsf/downloader/api.go
// Syncing provides information when this nodes starts synchronising with the Teslafunds network and when it's finished.
func (api *PublicDownloaderAPI) Syncing(ctx context.Context) (rpc.Subscription, error) {
	notifier, supported := rpc.NotifierFromContext(ctx)
	if !supported {
