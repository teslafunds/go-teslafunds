...
func NewGasPriceOracle(tsf *Teslafunds) *GasPriceOracle {
	minprice := tsf.GpoMinGasPrice
	if minprice == nil {
		minprice = big.NewInt(gpoDefaultMinGasPrice)
	}
	minbase := new(big.Int).Mul(minprice, big.NewInt(100))
	if tsf.GpobaseCorrectionFactor > 0 {
		minbase = minbase.Div(minbase, big.NewInt(int64(tsf.GpobaseCorrectionFactor)))
	}
...
func (gpo *GasPriceOracle) init() {
	gpo.initOnce.Do(func() {
		gpo.processPastBlocks(gpo.tsf.BlockChain())
		go gpo.listenLoop()
	})
}
...
func (self *GasPriceOracle) listenLoop() {
	events := self.tsf.EventMux().Subscribe(core.ChainEvent{}, core.ChainSplitEvent{})
...
func (self *GasPriceOracle) processBlock(block *types.Block) {
	...
	if lastBase.Cmp(lp) < 0 {
		corr = self.tsf.GpobaseStepUp
	} else {
		corr = -self.tsf.GpobaseStepDown
	}
...
func (self *GasPriceOracle) lowestPrice(block *types.Block) *big.Int {
	...
	receipts := core.GetBlockReceipts(self.tsf.ChainDb(), block.Hash())
	if len(receipts) > 0 {
	...
	if new(big.Int).Mul(gasUsed, big.NewInt(100)).Cmp(new(big.Int).Mul(block.GasLimit(),
		big.NewInt(int64(self.tsf.GpoFullBlockRatio)))) < 0 {
...
func (self *GasPriceOracle) SuggestPrice() *big.Int {
	...
	price.Mul(price, big.NewInt(int64(self.tsf.GpobaseCorrectionFactor)))
	price.Div(price, big.NewInt(100))
	if price.Cmp(self.minPrice) < 0 {
		price.Set(self.minPrice)
	} else if self.tsf.GpoMaxGasPrice != nil && price.Cmp(self.tsf.GpoMaxGasPrice) > 0 {
		price.Set(self.tsf.GpoMaxGasPrice)
	}
	return price
}
