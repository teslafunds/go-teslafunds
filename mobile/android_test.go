...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		AccountManager am = new AccountManager(getInstrumentation().getContext().getFilesDir() + "/keystore", Gdbix.LightScryptN, Gdbix.LightScryptP);
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gdbix-android-")
	if err != nil {
	...
	// Generate the mobile bindings for Gdbix and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.dubaicoin", "github.com/dubaicoin-dbix/go-dubaicoin/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gdbix.aar"), "gdbix.aar", os.ModePerm)
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.dubaicoin.gethtest"
