...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.teslafunds", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gtsf-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/teslafunds/gethtest", "libs"} {
	...
	build.CopyFile(filepath.Join("libs", "gtsf.aar"), "gtsf.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "teslafunds", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.teslafunds.gethtest"
