...
type NodeConfig struct {
	...
	// EthereumEnabled specifies whether the node should run the Dubaicoin protocol.
	EthereumEnabled bool

	// EthereumNetworkID is the network identifier used by the Dubaicoin protocol to
...
// Node represents a Gdbix Ethereum node instance.
type Node struct {
	node *node.Node
}

// NewNode creates and configures a new Gdbix node.
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	// Register the Dubaicoin protocol if requested
...
// GetEthereumClient retrieves a client to access the Dubaicoin subsystem.
