...
type Config struct {
	Oracle common.Address // Teslafunds address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Teslafunds service dependency to access the blockchain
	var teslafunds *tsf.Teslafunds
	if err := ctx.Service(&teslafunds); err != nil {
		return nil, err
	}
	// Construct the release service
	contract, err := NewReleaseOracle(config.Oracle, tsf.NewContractBackend(teslafunds))
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		case <-timer.C:
			...
			if version.Major > r.config.Major ||
				...
				howtofix := fmt.Sprintf("Please check https://github.com/teslafunds/go-teslafunds/releases for new releases")
