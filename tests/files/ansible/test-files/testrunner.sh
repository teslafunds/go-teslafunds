...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-teslafunds/build/test/createRandomStateTest\" teslafunds/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pyethereum
	OUTPUT_PYTHON="$(docker run --rm teslafunds/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm teslafunds/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm teslafunds/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
