...
import (
	...
	"github.com/teslafunds/go-teslafunds/common/registrar"
	"github.com/teslafunds/go-teslafunds/core"
	"github.com/teslafunds/go-teslafunds/crypto"
	"github.com/teslafunds/go-teslafunds/eth"
	...
	xe "github.com/teslafunds/go-teslafunds/xeth"
)
...
func (self *testFrontend) UnlockAccount(acc []byte) bool {
	self.teslafunds.AccountManager().Unlock(common.BytesToAddress(acc), "password")
	return true
}
...
func testExp(t *testing.T) (teslafunds *tsf.Teslafunds, err error) {
	...
	return tsf.New(&node.ServiceContext{EventMux: new(event.TypeMux)}, &tsf.Config{
		AccountManager:          am,
...
func testInit(t *testing.T) (self *testFrontend) {
	// initialise and start minimal teslafunds stack
	teslafunds, err := testExp(t)
	if err != nil {
		t.Errorf("error creating teslafunds: %v", err)
		return
	}
	err = teslafunds.Start(nil)
	if err != nil {
		t.Errorf("error starting teslafunds: %v", err)
		return
	}
	...
	self.wait = self.xtsf.UpdateState()
	addr, _ := self.teslafunds.Etherbase()
	...
	recG := self.xtsf.GetTxReceipt(common.HexToHash(txG))
	if recG == nil {
	...
	recH := self.xtsf.GetTxReceipt(common.HexToHash(txH))
	if recH == nil {
	...
	recU := self.xtsf.GetTxReceipt(common.HexToHash(txU))
	if recU == nil {
...
func TestNatspecE2E(t *testing.T) {
	...
	tf := testInit(t)
	defer tf.teslafunds.Stop()
	addr, _ := tf.teslafunds.Etherbase()
	...
	codeb := tf.xtsf.CodeAtBytes(registrar.HashRegAddr)
...
func pendingTransactions(repl *testFrontend, t *testing.T) (txc int64, err error) {
	txs := repl.teslafunds.TxPool().GetTransactions()
...
func processTxs(repl *testFrontend, t *testing.T, expTxc int) bool {
	...
	err = repl.teslafunds.StartMining(runtime.NumCPU(), "")
	if err != nil {
		t.Errorf("unexpected error mining: %v", err)
		return false
	}
	defer repl.teslafunds.StopMining()

	timer := time.NewTimer(100 * time.Second)
	height := new(big.Int).Add(repl.xtsf.CurrentBlock().Number(), big.NewInt(1))
	repl.wait <- height
	select {
	case <-timer.C:
