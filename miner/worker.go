...
func newWorker(config *core.ChainConfig, coinbase common.Address, tsf core.Backend) *worker {
	worker := &worker{
		config:         config,
		tsf:            tsf,
		mux:            tsf.EventMux(),
		...
		chain:          tsf.BlockChain(),
		proc:           tsf.BlockChain().Validator(),
	...
	for {
		for result := range self.recv {
			...
			if self.fullValidation {
				...
				auxValidator := self.tsf.BlockChain().AuxValidator()
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	accounts := self.tsf.AccountManager().Accounts()
...
func (self *worker) commitNewWork() {
	...
	txs := types.NewTransactionsByPriceAndNonce(self.tsf.TxPool().Pending())
	work.commitTransactions(self.mux, txs, self.gasPrice, self.chain)
	self.tsf.TxPool().RemoveBatch(work.lowGasTxs)
	self.tsf.TxPool().RemoveBatch(work.failedTxs)
