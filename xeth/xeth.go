...
// Package xeth is the interface to all Teslafunds functionality.
package xeth
...
type XEth struct {
	...
	// read-only fields
	backend       *tsf.Teslafunds
	frontend      Frontend
	agent         *miner.RemoteAgent
	gpo           *tsf.GasPriceOracle
	state         *State
	whisper       *Whisper
	filterManager *filters.FilterSystem
}

func NewTest(tsf *tsf.Teslafunds, frontend Frontend) *XEth {
	return &XEth{backend: tsf, frontend: frontend}
}

// New creates an XEth that uses the given frontend.
// If a nil Frontend is provided, a default frontend which
// confirms all transactions will be used.
func New(teslafunds *tsf.Teslafunds, frontend Frontend) *XEth {
	xeth := &XEth{
		backend:          teslafunds,
		frontend:         frontend,
		quit:             make(chan struct{}),
		filterManager:    filters.NewFilterSystem(teslafunds.EventMux()),
		logQueue:         make(map[int]*logQueue),
		blockQueue:       make(map[int]*hashQueue),
		transactionQueue: make(map[int]*hashQueue),
		messages:         make(map[int]*whisperFilter),
		agent:            miner.NewRemoteAgent(),
		gpo:              tsf.NewGasPriceOracle(teslafunds),
	}
	if teslafunds.Whisper() != nil {
		xtsf.whisper = NewWhisper(teslafunds.Whisper())
	}
	teslafunds.Miner().Register(xtsf.agent)
	if frontend == nil {
		xtsf.frontend = dummyFrontend{}
	}
	state, _ := xtsf.backend.BlockChain().State()
	xtsf.state = NewState(xeth, state)
	go xtsf.start()
	return xeth
}
...
func (self *XEth) WithState(statedb *state.StateDB) *XEth {
	...
	xtsf.state = NewState(xeth, statedb)
	return xeth
}
