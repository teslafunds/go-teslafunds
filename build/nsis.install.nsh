Name "gdbix ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install gdbix binary
Section "Gdbix" GETH_IDX
  SetOutPath $INSTDIR
  file {{.Gdbix}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\gdbix.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\gdbix.exe" "attach" "" ""
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "" ""

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "Gdbix incoming peers (TCP:57955)"
  SimpleFC::AdvRemoveRule "Gdbix outgoing peers (TCP:57955)"
  SimpleFC::AdvRemoveRule "Gdbix UDP discovery (UDP:57955)"

  # Firewall - add rules
  SimpleFC::AdvAddRule "Gdbix incoming peers (TCP:57955)" ""  6 1 1 2147483647 1 "$INSTDIR\gdbix.exe" "" "" "Dubaicoin" 57955 "" "" ""
  SimpleFC::AdvAddRule "Gdbix outgoing peers (TCP:57955)" ""  6 2 1 2147483647 1 "$INSTDIR\gdbix.exe" "" "" "Dubaicoin" "" 57955 "" ""
  SimpleFC::AdvAddRule "Gdbix UDP discovery (UDP:57955)" "" 17 2 1 2147483647 1 "$INSTDIR\gdbix.exe" "" "" "Dubaicoin" "" 57955 "" ""

  # Set default IPC endpoint (https://github.com/ethereum/EIPs/issues/147)
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "R" "HKLM" "\\.\pipe\gdbix.ipc"
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "A" "HKLM" "\\.\pipe\gdbix.ipc"
