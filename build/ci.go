...
import (
	...
	"github.com/teslafunds/go-teslafunds/internal/build"
)

var (
// Files that end up in the gtsf*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gtsf"),
	}

// Files that end up in the gtsf-alltools*.zip archive.
...
// A debian package is created for all executables listed here.
	debExecutables = []debExecutable{
		...
		{
			Name:        "evm",
			Description: "Developer utility version of the EVM (Teslafunds Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			Name:        "gtsf",
		...
		{
			Name:        "puppeth",
			Description: "Teslafunds private network manager.",
		},
		{
			Name:        "rlpdump",
		...
		{
			Name:        "swarm",
			Description: "Teslafunds Swarm daemon and tools",
		},
		{
			Name:        "wnode",
...
func doInstall(cmdline []string) {
	...
	if runtime.Version() < "go1.7" && !strings.Contains(runtime.Version(), "devel") {
		...
		log.Println("go-teslafunds requires at least Go version 1.7 and cannot")
...
func doArchive(cmdline []string) {
	...
	var (
		...
		gtsf     = "gtsf-" + base + ext
		alltools = "gtsf-alltools-" + base + ext
	)
	// maybeSkipArchive(env)
	if err := build.WriteArchive(gtsf, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gtsf, alltools} {
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Where to upload the source package (usually "ppa:teslafunds/teslafunds")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gtsf-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time) debMetadata {
	if author == "" {
		...
		author = "Teslafunds Builds <christopher@teslafunds.tech>"
	}
	return debMetadata{
		Env:         env,
		Author:      author,
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gtsf.exe" {
			gethTool = file
		} else {
	...
	installer, _ := filepath.Abs("gtsf-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gtsf.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "--target", "android", "--javapkg", "org.teslafunds", "-v", "github.com/teslafunds/go-teslafunds/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gtsf.aar", filepath.Join(GOBIN, "gtsf.aar"))
		return
	}
	...
	archive := "gtsf-" + archiveBasename("android", env) + ".aar"
	os.Rename("gtsf.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "--target", "ios", "--tags", "ios", "-v", "github.com/teslafunds/go-teslafunds/mobile")
	...
	archive := "gtsf-" + archiveBasename("ios", env)
