...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gdbix is "gdbix". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gdbix", keep that.
	if name == "gdbix" || name == "gdbix-testnet" {
		name = "Gdbix"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gdbix" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	// by gdbix 1.4 are used if they exist.
	if c.name() == "gdbix" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "gdbix" {
