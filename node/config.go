...
import (
	...
	"github.com/teslafunds/go-teslafunds/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gtsf is "gtsf". If no
...
// These resources are resolved differently for "gtsf" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	// by gtsf 1.4 are used if they exist.
	if c.name() == "gtsf" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "gtsf" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-teslafunds-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
