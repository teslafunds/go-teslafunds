...
<<<<<<< HEAD
	...
	"github.com/teslafunds/go-teslafunds/p2p/nat"
	...
	"github.com/teslafunds/go-teslafunds/whisper"
=======
...
>>>>>>> 7fdd714... gdbix-update v1.5.0
	...
	IPCPathFlag = DirectoryFlag{
		...
		Value: DirectoryString{"gdbix.ipc"},
	}
...
// for Gtsf and returns half of the allowance to assign to the database.
...
<<<<<<< HEAD
	...
	// Configure the Teslafunds service
...
<<<<<<< HEAD
	case ctx.GlobalBool(OlympicFlag.Name):
		...
		tsfConf.Genesis = core.OlympicGenesisBlock()

	case ctx.GlobalBool(TestNetFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			tsfConf.NetworkId = 2
		}
		tsfConf.Genesis = core.TestNetGenesisBlock()
		state.StartingNonce = 1048576 // (2**20)

	case ctx.GlobalBool(DevModeFlag.Name):
		// Override the base network stack configs
		if !ctx.GlobalIsSet(DataDirFlag.Name) {
			stackConf.DataDir = filepath.Join(os.TempDir(), "/teslafunds_dev_mode")
		}
		...
		if !ctx.GlobalIsSet(GasPriceFlag.Name) {
			tsfConf.GasPrice = new(big.Int)
		}
		if !ctx.GlobalIsSet(WhisperEnabledFlag.Name) {
			shhEnable = true
		}
		tsfConf.PowTest = true
	}
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return tsf.New(ctx, tsfConf)
	}); err != nil {
		Fatalf("Failed to register the Teslafunds service: %v", err)
=======
	case ctx.GlobalBool(TestNetFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			ethConf.NetworkId = 7996
		}
	...
	// Override any global options pertaining to the Dubaicoin protocol
...
>>>>>>> 7fdd714... gdbix-update v1.5.0
	...
	if ethConf.LightMode {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			...
			Fatalf("Failed to register the Dubaicoin light node service: %v", err)
		}
	} else {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			...
			Fatalf("Failed to register the Dubaicoin full node service: %v", err)
		}
	}
}
...
// RegisterEthStatsService configures the Dubaicoin Stats daemon and adds it to
// th egiven node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		var lesServ *les.LightDubaicoin
		ctx.Service(&lesServ)

		return ethstats.New(url, ethServ, lesServ)
	}); err != nil {
<<<<<<< HEAD
		Fatalf("Failed to register the Gtsf release oracle service: %v", err)
=======
		Fatalf("Failed to register the Dubaicoin Stats service: %v", err)
>>>>>>> 7fdd714... gdbix-update v1.5.0
	}
}
...
func MakeChainConfigFromDb(ctx *cli.Context, db ethdb.Database) *params.ChainConfig {
	...
	//  - If no genesis is set, we're running either mainnet or testnet (private nets use `gdbix init`)
